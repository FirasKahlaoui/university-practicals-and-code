...
SNR_dB   = 10;  % Rapport signal/bruit du canal FM (dB) — modifier ici
duration = 0.005;
...
fc         = 1000000;      % ← 1 MHz au lieu de 100 kHz
...
%% 0. Ajout de bruit AWGN sur le signal FM
% Puissance du signal s(t) (amplitude 1 → Ps = 0.5)
Ps        = mean(s.^2);
Pn        = Ps / (10^(SNR_dB/10));          % puissance du bruit cible
bruit     = sqrt(Pn) * randn(size(s));       % bruit blanc gaussien
s_bruite  = s + bruit;                       % signal FM bruité

fprintf('AWGN ajouté : SNR = %d dB  |  Pb = %.2e\n', SNR_dB, Pn);
...
s_filt  = filtfilt(bpf_pre, 1, s_bruite);
...
hold on
plot(T, s_bruite, 'LineWidth', 0.8, 'DisplayName', sprintf('s(t) bruité (SNR=%d dB)', SNR_dB))
